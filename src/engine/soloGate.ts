...
export function evaluateSoloGate(inputs: SoloGateInputs) {
  ...
  const patternAvg = avg(lastNScoresFor(inputs.lessons, PATTERN_IDS, windowN));
  ...
  const eligible = meetsPattern && meetsEmergency && meetsMedical && meetsTsa;

  const reasons: string[] = [];

  if (!meetsPattern)
    reasons.push(
      `Pattern avg ≥ 4 (last ${windowN}: ${patternAvg?.toFixed(2) ?? "N/A"})`,
    );
  ...
  if (!meetsMedical) reasons.push("Medical must be completed");

  if (!meetsTsa) reasons.push("TSA (A.14) must be completed");
